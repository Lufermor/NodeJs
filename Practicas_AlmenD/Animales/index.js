...
app.get("/index2", (req, res) => {
  res.render("index2.ejs", {api, permisos});
});
...
Por favor escribe el código para tres tablas mysql:
Tabla equipo:
  - id autoincrement
  - nombre varchar
  - fundación Date
...
app.post("/changeApi", (req, res) => {
  api = generateApikey.generateApiKey();
  api = api.replaceAll("/","(");
  con.query(`UPDATE usuario SET api = '${api}' WHERE (id = ${idUser});`, 
  function (err, result) {
  });
  res.redirect("index2");
});
...
app.get("/:api/mascotas/:id", (req, res) => {
  ...
  con.query(`select * from usuario where '${req.params.api}' = api`,  function (err, result) {
    ...
    if(result.length < 1){
      bol = false;
      console.log(" menor de 1 false " + bol);
    }
    else{
      bol = true;
      console.log(" mayor de 1 true " + bol);
    }
    if(bol == true){
...
app.get("/:api/mascotas/protectora/:id", (req, res) => {
  ...
  con.query(`select * from usuario where '${req.params.api}' = api`,  function (err, result) {
    ...
    if(result.length < 1){
      bol = false;
      res.send("La api no consta en la base de datos")
    }
    else{
  ...
  function (accessToken, refreshToken, profile, done) {
    ...
    con.query(`select * from usuario where '${idUser}' = id`, function (err, result) {
      ...
      if (result.length == 0) {
        ...
        values
        (${idUser},'${userLogin}','${api}', ${permisos})`, function (err, result) {
